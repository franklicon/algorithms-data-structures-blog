      ...
      <div className="border border-rule dark:border-graphite/60 rounded-lg p-6 lg:p-8 bg-paper dark:bg-graphite/20">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-8">
          <input
            ...
            placeholder="target"
            disabled={isPlaying}
            className="w-24 px-3 py-2 font-mono text-sm border border-rule dark:border-graphite/60 bg-transparent text-ink dark:text-paper rounded focus:outline-none focus:border-ember disabled:opacity-50"
          />
            ...
            {items.map((value, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-0.5 h-4 mb-1 font-mono text-[10px] font-bold tracking-tight text-ember">
                    {isLeft && <span>L</span>}
                    {isMid && <span>M</span>}
                    {isRight && <span>R</span>}
                  </div>
                  ...
                  {/* Index */}
                  <span className="font-mono text-[9px] text-ink/40 dark:text-paper/40 mt-1 tabular-nums">
                    {idx}
                  </span>
                </div>
              );
        ...
        <div className="mt-6 pt-4 border-t border-rule dark:border-graphite/60 flex justify-between items-center font-mono text-[11px] text-ink/50 dark:text-paper/50">
          <span className="text-ink/80 dark:text-paper/80 min-h-[1em]">
            {result}
          </span>
          <span>
            {stepCount !== null
